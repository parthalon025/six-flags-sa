...
export function createParty({ id, name = 'Party', leader, now = Date.now(), transport = null }) {
  return {
    ...
    rides: {},
    meet: null,
    settings: { shareLocationHistory: false },
  };
}
...
export function reduce(state, command, now = Date.now()) {
  ...
  switch (kind) {
    ...
    case 'patch-member': {
      ...
      if (body.patch?.avatar !== undefined) patch.avatar = body.patch.avatar;
    ...
    case 'set-meet': {
      const meet = body.meet ? { ...body.meet, by: me?.name || 'Someone', ts: now } : null;
