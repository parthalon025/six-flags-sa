...
/**
 * Cadence for a page nobody is looking at.
 *
 * A phone in a pocket is the common case for hours at a time, and a poll it
 * generates is a request nobody reads: the roster it fetches is drawn to a
 * screen that is off. Browsers already clamp background timers, so this is less
 * a change in behaviour than an honest statement of it — the difference is that
 * backing off deliberately means the party is caught up the moment the screen
 * comes back, instead of whenever a throttled timer happens to fire next.
 */
const POLL_HIDDEN_MS = 15000;

/** True only where a document exists and is currently out of sight. */
...
export function createMailboxChannel({
   ...
   */
  const delay = () => {
    if (hidden()) return Math.max(pollMs, POLL_HIDDEN_MS);
    return Date.now() - lastTraffic < HOT_WINDOW_MS ? Math.min(POLL_HOT_MS, pollMs) : pollMs;
  };
  ...
  /**
   * A screen coming back on is the one moment a stale roster is actually looked
   * at, so the backed-off timer is cut short rather than waited out. Without
   * this, unlocking the phone would show the party as it stood up to
   * POLL_HIDDEN_MS ago and call that live. A poll already in flight leaves
   * `timer` null, and there is nothing to hurry along in that case.
   */
  function wake() {
    if (stopped || hidden() || mode !== 'polling' || !timer) return;
    clearTimeout(timer);
