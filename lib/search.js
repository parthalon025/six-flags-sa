...
const contains = (text, q) =>
  q.length <= 4
    ? new RegExp(`\\b${esc(q)}`, 'i').test(text || '')
 ...
 */
export function matchesQuery(poi, query, cats) {
  const q = (query || '').trim().toLowerCase();
  if (!q) return true;
  if (contains(poi.n, q) || contains(poi.a, q)) return true;
...
export function matchedByName(poi, query) {
  const q = (query || '').trim().toLowerCase();
  if (!q) return false;
  return contains(poi.n, q) || contains(poi.a, q);
}
