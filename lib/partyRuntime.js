 ...
 *
 * On the key. A six-character code has to be enough to join, because reading a
 * 256-bit key aloud in a queue is not a thing anyone will do. So the party key
 * is derived from `partyId + code` rather than minted at random: both the
 * creator and a code-only joiner can compute it, and the relay — which sees the
 * party id but never the code — still cannot. The trade is honest and it is the
 * same one the README already documents: a party is a semi-public channel whose
 * secrecy is exactly the secrecy of its code. The invite link carries the
 * derived key in its fragment anyway, so a link-join needs no code lookup at
 * all and works with no server in the path.
 */
...
import { b64urlEncode, importKey } from '@/lib/core/crypto';
...
export function createPartyRuntime({ onState = noop, onStatus = noop, onToast = noop } = {}) {
  ...
  /**
   * Deterministic party key. Binding it to the party id as well as the code
   * means a recycled code (they expire and are reissued) never reproduces an
   * older party's key.
   */
  async function deriveKeyString(partyId, code) {
    const seed = new TextEncoder().encode(`ki-party-v3|${partyId}|${normalizeCode(code)}`);
  ...
  function createLink(mgr) {
    ...
    return {
      ...
      drain: () => pending.catch(() => null),
    };
  }

  function buildTransports(role) {
    // Rank order is a property of the transports themselves (LOCAL_HTTP 10,
    // WEBRTC 20, BLUETOOTH 30, CLOUD_RELAY 40, OFFLINE 99); the manager sorts
    // on it. Listing them in the same order is for the reader.
  ...
  function openManager(role) {
    manager = createTransportManager({
      ...
      session,
      onMessage: (sealed) => service()?.handleSealed?.(sealed),
      onStatus: (event) => {
   ...
   */
  function seedHost(service_, snapshot) {
    const state = service_.getState();
    Object.assign(state, {
      ...snapshot,
  ...
  async function teardown({ announce = false } = {}) {
    if (announce) {
      client?.stop(); // posts BYE, which deletes the record instead of waiting out the TTL
      host?.stop();
      await link?.drain();
    }
    link?.mute();
  ...
  async function begin(built, role, memberName, partyName) {
    session = built;
  ...
  async function createParty({ name = 'Party', memberName = 'Guest' } = {}) {
    ...
    try {
      ...
      const keyString = await deriveKeyString(allocated.partyId, allocated.code);
    ...
    try {
      ...
      if (!bundle) {
        ...
        bundle = {
          partyId,
          code,
          keyString: await deriveKeyString(partyId, code),
          token: '',
          endpoints: [],
        };
      }
      ...
      return await begin(built, 'client', memberName, 'Party');
    } catch (err) {
      phase = 'error';
      error = String(err?.message || err);
      emit();
