...
export function createElection({
  ...
  function beginElection() {
    ...
    // Armed before the claim goes out: a still-living host answers a claim
    // synchronously on a local transport, and the cancellation that follows has
    // to be able to find this timer.
    dbg('beginElection', selfId, 'score', mine.score, 'jo', mine.joinOrder, 'silentFor', now() - lastHostSeen);
  ...
  function resolve() {
    ...
    dbg('resolve', selfId, 'claims', [...claims.keys()].join(','), '-> winner', best.id);
    cancelElection();
    if (best.id === selfId) {
      promote();
      return;
  ...
  function handleClaim(f) {
    dbg('handleClaim', selfId, 'from', f?.from);
    ...
    if (promoted) {
      ...
      if (beats(mine, rival) && now() - lastVictorySentAt >= reassertGapMs) sendVictory();
      return;
    }
    if (!electing) {
      ...
      if (now() - lastHostSeen <= Math.max(0, hostTimeoutMs - claimWindowMs)) {
        dbg('claim ignored (guard)', selfId, 'from', rival.id, 'silentFor', now() - lastHostSeen);
        return;
      }
      beginElection();
      if (!electing) return;
    }
    dbg('claim accepted', selfId, 'from', rival.id);
  ...
  function handleVictory(f) {
    ...
    dbg('handleVictory', selfId, 'from', theirs.id, 'promoted', promoted);
    if (!theirs.id) return;
    const mine = myClaim || selfClaim();
