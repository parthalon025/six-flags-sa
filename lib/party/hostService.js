...
export function createHostService({
  ...
  async function handleSealed(sealed) {
    try {
      ...
      if (ELECTION_KINDS.has(f.kind)) dbg('host rx', f.kind, 'from', f.from, 'seq', f.seq);
      if (!dedupe.accept(f.from, f.seq)) {
        dbg('host dedupe reject', f.kind, 'from', f.from, 'seq', f.seq);
      ...
      if (ELECTION_KINDS.has(f.kind)) {
        ...
        dbg('host emits election', f.kind, 'from', f.from);
