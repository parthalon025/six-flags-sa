...
export function createClient({ session, key, transport, now = () => Date.now() }) {
  ...
  function post(kind, body, to) {
    ...
    outbound = outbound
      .then(async () => {
        ...
        const r = await transport.send(sealed);
        if (kind === CLAIM || kind === VICTORY) dbg('client tx', kind, 'result', JSON.stringify(r), 'active', transport.activeName?.());
        counters.sent += 1;
      })
      .catch(fail);
    return outbound;
  ...
  async function handleSealed(sealed) {
    try {
      ...
      if (f.kind === CLAIM || f.kind === VICTORY || f.kind === PING) dbg('client rx', f.kind, 'from', f.from, 'seq', f.seq);
      if (!dedupe.accept(f.from, f.seq)) {
        dbg('client dedupe reject', f.kind, 'from', f.from, 'seq', f.seq);
