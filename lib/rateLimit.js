...
export const LIMITS = {
  ...
  // Keyed by IP, and deliberately counting mailboxes brought into existence
  // rather than messages. A party created while the API was unreachable has a
  // client-minted id and no server record (lib/partyRuntime.js `allocate`), so
  // the relay cannot demand one exist — but such a party creates exactly one
  // mailbox, and a script pointed at the endpoint creates thousands. Metering
  // the creation separates them without metering the messages, which a host
  // beacon alone would blow through every hour.
  mailboxCreate: { limit: 20, windowMs: 60 * 60 * 1000 },
