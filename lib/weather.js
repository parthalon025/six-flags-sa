...
const SHELTERED_CATEGORIES = new Set(['food', 'shop', 'restroom', 'service']);

/** Categories that are geography, not something that opens or closes. */
const INERT_CATEGORIES = new Set(['gate', 'parking', 'landmark']);

const RIDE_CATEGORIES = new Set(['coaster', 'ride']);
...
export function classifyWeather(obs) {
  ...
  return {
    ...
    obs: { code, gust, temp, precip, chance, storming, windy, wetNow, cold, hot },
  };
}
