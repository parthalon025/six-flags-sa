...
// query rather than a full scan. Nothing in this file reads, logs, validates or
// transforms `data`.
...
/** Internal alias; `redisCommand` is exported for lib/rateLimit.js. */
...
/**
 * Does this party exist, without paying to read it.
 *
 * Deliberately not `readParty`: this answers a gatekeeping question on the
 * mailbox write path, where the record itself is not wanted and the eviction
 * write-back that a real read performs would be waste. One EXISTS, no parse.
 */
export async function partyExists(id) {
  if (usingRedis) return Number(await redis(['EXISTS', partyKey(id)])) === 1;
  const party = mem.parties.get(id);
...
/**
 * Everything in this mailbox after `since`.
 *
 * The cursor is pushed down to the store rather than applied after the fact.
 * The messages are a sorted set scored by seq precisely so this can be a range
 * query: a caller that is caught up transfers nothing, instead of dragging the
 * whole box across the wire on every poll to discard it. That distinction is
 * the difference between a poll costing O(new) and O(depth), and a party of six
 * polling every 2.5s makes that the hot path of the entire deployment.
 *
 * @returns `{ messages, seq }` — `seq` is the high-water mark, expired or not.
 */
export async function readMailbox(id, since = 0) {
  const now = Date.now();
  const from = Number.isFinite(since) && since > 0 ? since : 0;

  if (usingRedis) {
    const [rows, high] = await pipeline([
      // `(from` is an exclusive minimum: strictly newer than the caller's cursor.
      ['ZRANGEBYSCORE', boxKey(id), `(${from}`, '+inf'],
      ['GET', seqKey(id)],
    ]);
    const messages = (rows || [])
      .map((row) => {
  ...
  return { messages: kept.filter((m) => m.seq > from), seq: box.seq };
}
 ...
 *
 * The write path needs to tell "another message for an ongoing conversation"
 * from "the request that brings a new key into being", because only the second
 * is a way for an anonymous caller to make this store grow.
 */
export async function mailboxExists(id) {
  if (usingRedis) return Number(await redis(['EXISTS', boxKey(id)])) === 1;
  return mem.boxes.has(id);
}
...
/**
 * Does this party already have a subscription list.
 *
 * Same question `mailboxExists` answers, for the other key an unauthenticated
 * caller can bring into being: is this request joining something that is
 * already here, or creating something that is not.
 */
export async function subscriptionsExist(partyId) {
  if (!partyId) return false;
  if (usingRedis) return Number(await redis(['EXISTS', subsKey(partyId)])) === 1;
  return (mem.subs.get(partyId)?.size ?? 0) > 0;
}
