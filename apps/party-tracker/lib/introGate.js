 ...
 * answered, then the Profile gate (Clerk login / guest) → welcome inside that
 * same opaque shell.
 *
 * `null` is the SSR / pre-hydration answer — storage cannot be read on the
 * server, and guessing either way flashes the wrong card.
 *
 * @param {{ introSeen: boolean | null }} state
 * @returns {'hold' | 'welcome' | 'none'}
 */
export function firstRunOverlay({ introSeen }) {
