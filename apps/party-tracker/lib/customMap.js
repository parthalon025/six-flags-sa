 ...
 * with the iso flavour painted into the sprites plus a camera preset. ADR-0021
 * reaffirms the rejection of keeping a true-iso path for it — "if painted-iso
 * plus a camera preset cannot carry the feeling, the answers are a kit redesign
 * or retiring the Skin with compensation, not two renderers forever".
 *
  ...
  if (spec.camera != null && !CUSTOM_MAP_CAMERAS.includes(spec.camera)) {
    throw new Error(
      `custom map "${spec.id}" names camera "${spec.camera}"; the map path has `
        + `${CUSTOM_MAP_CAMERAS.join(', ')}. A Skin's camera FEEL is a preset on the `
        + 'shared camera curve (packages/shared/mapCamera.js), not a second projection',
    );
  }
  return spec;
}
