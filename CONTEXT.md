...
**Gap**:
A missing or unconfirmed fact the builder ships on a **Venue** (`*.gaps.json`) that open sources cannot settle. Types this ship: height, queue, restroom, food, gate, camping. Atomic on the wire (one **Gap** per fact; `target` is the **Place** key `i` when the **Place** exists, or null for a missing **Place** / camping). The builder invents **Gaps** once; the phone ranks them by **Location** and must not invent them from POI heuristics. Missing `gaps.json` is an empty list, not a failed **Venue** load. The phone may group cards (e.g. “Find the restrooms” with progress). **Gaps** seed **Side Quests**.
_Avoid_: Bug, missing POI (too vague); Contribution (the answer, not the hole)

**XP**:
A **Profile** progress score earned by completing **Side Quests**. Never spent. Rank-up is the reward. Repeat of the same (`venue`, `type`, `target`) by the same **Profile** awards 0. Not a public leaderboard.
_Avoid_: experience (that **Contribution** kind); points (generic)

**Rank**:
Status on a **Profile** granted by **XP** (visitor, scout, ranger, cartographer, steward). This ship shows Status on the signed-in **Profile**. Cartographer later unlocks full-ontology Create (Wayfarer) — not this Field Research loop.
_Avoid_: leaderboard; level (use **Rank**)
...
- Completing a gap **Side Quest** produces a **Contribution** and, when the **Profile** walked near (or is inside **Venue** bounds for camping / add-**Place**), awards **XP**
- Completing a live **Side Quest** produces a **Ride report** (name-first); it is not a **Contribution**; **XP** for a live report needs a **Profile**
...
- **XP** is never spent; **Rank** is the visible reward
...
> **Domain expert:** "No. The card says help other guests. **XP** lands on your **Profile**; **Scout** is the reward you see. Same (`venue`, type, **Place**) again is 0 — no farming."
>
...
- **XP** vs leaderboard — resolved: **XP** grants **Rank** rewards (Status). No all-time global leaderboard this ship. Cards stay meaning-first; rank-up may toast.
- Field Research vs Create (Q13) — resolved: this ship is Field Research chips (height / queue / restroom / food / gate / camping). Full-ontology Create is a later **Cartographer** / Wayfarer loop, not an “Other…” on Find-the-restrooms.
