...
**Rank prize**:
A **Skin** or **Kit** bundled with a **Rank** threshold — granted when **XP** crosses that **Rank**, never bought and never spent. The **Title** is separate; **Rank prizes** are listed in the Rank prize catalog (e.g. Scout → Porter cuff **Kit**; Ranger → Postcard **Skin**). Policy lives in `packages/shared/rankPrizes.js`.
_Avoid_: Ex prize; loot box; store purchase; Member reward
...
**Compass**:
Facing-relative instrument on the phone (bearing strip) and Apple Watch (dial) that answers where **Members**, the **Meet**, and the current primary **Place** (Go destination, map selection, or next **Plan** **Place**) are relative to the way you are facing — a game-style radar, not a north-first magnetic rose. Watch guests get full presentation settings (density, Always On, show party / Meet, units, turn haptics, raise-to-nav); mark priority stays one product language. The map-edge north rose is not this instrument.
_Avoid_: compass rose (retired map furniture for this job); bearing tape (legacy chrome name); magnetic compass; quest sensor
...
- The **Compass** is facing-relative on phone and Apple Watch; it shows **Members** with a **Location** pin, the **Meet**, and one primary **Place** (Go beats selection / Plan-next; selection replaces Plan-next outside Go)
- The park map stays north-up except during Go (course-up); the map-edge compass rose is not used — the **Compass** strip / Watch dial carry north as a quiet tick when useful
