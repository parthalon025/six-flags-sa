...
3. **Upgrade globally**, not in this repo — `node scripts/install-global-skills.mjs` (Cloud Agents run this from `.cursor/environment.json` `install` and `start` so `~/.cursor/skills` stays linked to the global Matt tree).
