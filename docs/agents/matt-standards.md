...
7. **Sonnet standards review before merge (code diffs)** — spawn a `claude-sonnet-5` subagent with the prompt from `node scripts/ci/matt-review.mjs prompt` (it applies `code-review`, this doc, `codebase-design`, and the [root-cause policy](./policies/root-cause.md); give it the GitNexus blast radius when the index is available). Address or answer its advisory findings, then stamp with `node scripts/ci/matt-review.mjs write --gitnexus <ok|unavailable>` and commit `scripts/ci/matt-review-pass.json`. CI and `test:pre-merge-vertical` fail code diffs without a fresh stamp; docs-only diffs are exempt.
8. **Root cause** — ship the layer that should have spoken. A hide is a merge only when that policy names the exception. See [root-cause policy](./policies/root-cause.md).
