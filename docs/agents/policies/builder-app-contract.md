...
- A one-off correction for a single venue (height, area, alias, hand-added place, recipe/box/sources) → fix that venue's own input under `packages/venue-builder/data/venues/<id>/` (`overrides.json`, `sources.json`, `recipe.json`, `ids.json`, `attractions.json`, `heights.json`), or — for how a Zone is painted — its land-cover harvest at `data/venues/<id>/display/grounding.json` and its hand-authored per-Zone character at `data/venues/<id>/display/zone-character.json` (harvest cannot overwrite the latter).
