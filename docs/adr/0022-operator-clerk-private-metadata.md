...
**Depends on:** [ADR-0030 Clerk profile signup](./0030-clerk-profile-signup.md), [ADR-0009 ship Gaps](./0009-ship-gaps.md)
...
1. Clerk Backend `private_metadata.admin === true` is the Operator bit. The phone never reads it. `POST /api/profile/sync` asks Clerk Backend and, when true, writes Steward / 3000 into Postgres (ADR-0030: Postgres still owns **XP** / **Title**).
...
5. Reset is taking `private_metadata.admin` off that Clerk user. Next sync stops granting. Clerk Organizations stay a non-goal (ADR-0030).
