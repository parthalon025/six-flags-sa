...
2. **Bands are specified in ground sample distance, on power-of-two steps.** Overview **2.4 m/px**
   ...
   row 5 has a real parent to upscale from. `px/cell` is retired as the specifying unit — it was
   never defined anywhere in the repo — and survives only as a derived note.
...
8. **Grounding covers the overview and mid bands only.** Recognition is an arm's-length
   ...
   stays out of scope while per-venue capture cost is a human walking the park.
...
- ADR-0019 clause 8's zero-blank-tiles row drops from a correctness gate to a polish gate: with
  Truth offline and paint carrying no facts, a blank tile is ugly, not wrong.
...
- `docs/adr/0016-custom-map-worlds.md`, the perf playbook and `CONTEXT.md` carry the old band units
  and the withdrawn on-wear sync; all three are corrected alongside this ADR.
...
- **Train H's build order.** #563 puts the two riskiest unknowns — does flat art read well
  pitched, does a mobile WebView hold up — in the last slices, and clause 6 above moves
  pixel-tycoon's conversion to the front. The order needs redrawing; no decision taken here.
- **The perf gate rows.** The review established that background drain is the real concern (the
  ...
  real device joins the gate, or what a drain budget would be.
