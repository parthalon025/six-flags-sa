...
/* A Skin name with no declared camera preset, deliberately. Since slice h14 the
   seam derives its pitch ceiling from the Skin, so a fixture naming a SHIPPED
   Skin would silently import that Skin's design decision into every case below
   — and the curve cases here are about the curve, not about anyone's feel. The
   preset derivation gets its own block further down, where the Skin is the
...

{
  ...
  assert.equal(attached.view.skin, 'fixture-skin');
...
/* And the Skin does not have to be asked twice (slice h14). ADR-0019 clause 2
   makes the pitch preset a per-Skin declared trait, so a caller that names the
   Skin has already named the feel — a second argument repeating it is a second
   place for the two to disagree, and the way a Skin quietly mounts on the
   default camera. An explicit maxPitch still wins: the seam derives a default,
   it does not overrule its caller. */
{
  const declared = mount({ skin: 'watercolor-quest' });
