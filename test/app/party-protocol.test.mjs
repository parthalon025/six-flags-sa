...
await check('two phones that promote at once settle on one host via the total order', async () => {
  // #594: reconcile must apply outranks, not the steal margin, when two live
  // hosts hear each other after simultaneous promotion.
  const party = await makeParty({ clients: 2 });
  try {
    party.bus.partition('phone-a');
    ...
    assert.equal(stillHosting.length, 1, 'split brain did not resolve');
    const loser = party.clients.find((p) => !p.host);
    assert.ok(loser?.events.includes('step-down'), 'the loser did not step down');
  } finally {
    await party.teardown();
  }
});

await check('host reconciliation yields on outranks, not the steal margin', async () => {
  // #594: join-order tiebreaks separate two hosts; shouldYield margin is for steals.
  const { scoreCandidate, outranks } = await import(`${APP}lib/party/election.js`);
...
const TRANSCRIBED_FROM = [
  ...
  ['reconcile', 'partyRuntime.js:598-627', '7563e6e6b8947ab2'],
