...
const { outranks, readRank, shouldYield } = await import(`${APP}lib/party/election.js`);
...
await check('two phones that promote at once reconcile to one host', async () => {
  const party = await makeParty({ clients: 2 });
  try {
    party.bus.partition('phone-a');
    ...
    assert.equal(stillHosting.length, 1, 'split brain should resolve to one host');
    const steppedDown = party.clients.filter((p) => p.events.includes('step-down'));
    assert.equal(steppedDown.length, 1, 'the lower-ranked host should step down');
  } finally {
    await party.teardown();
  }
});

await check('outranks names a winner that reconcile ratifies without a battery margin', async () => {
  ...
  // Steal margin still requires a battery gap; tiebreak alone is not enough.
  assert.equal(shouldYield(second, first), false);
  assert.equal(shouldYield(first, second), false);
  assert.ok(first.score - second.score < 1);
  assert.ok(STEAL_STEPS * (1e12 / 100) > 1e10);
  // Reconciliation uses outranks directly — the loser yields.
  assert.equal(outranks(second, first), false);
  assert.equal(outranks(first, second), true);
});
...
const TRANSCRIBED_FROM = [
  ...
  ['reconcile', 'partyRuntime.js:598-627', '075c6dcd24aeaf19'],
