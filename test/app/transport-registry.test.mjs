...
await check('a host on LAN warms the cloud mailbox for joiners on mobile data (#595)', async () => {
  // A host reachable over the self-hosted LAN mailbox must still keep the
  // relay inbox warm so a joiner on mobile data can post HELLO.
  const cloud = relay();
  const local = lan();
  ...
  assert.deepEqual(mgr.desiredWarm().map((t) => t.name), ['relay']);
  await mgr.close();
});

await check('connect opens the active transport exactly once (#595)', async () => {
  const local = lan();
