...
await check('a host on LAN also warms the cloud mailbox for joiners on mobile data', async () => {
  // #595: the host must listen on the relay even when LAN is the send path, so
  // a joiner on mobile data is not posting HELLO into an empty mailbox.
  const cloud = relay();
  const local = lan();
...
await check('connect opens the active transport once, not as warm and send', async () => {
  // #595: pre-selection warmth must not claim the inbox slot the selection loop
