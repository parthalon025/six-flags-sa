#!/usr/bin/env node
/**
 * /api/observations route: POST append and GET read (#327 / E7.1).
 *
 * Seams:
 *   POST — validated append-only observation row
 *   GET  — by id, or list filtered by venueId / placeId
 *
...
await check('POST rejects invalid observation shape with 400', async () => {
  resetStore();
...
await check('POST appends observation and returns 201', async () => {
  resetStore();
...
await check('GET by venueId returns appended row', async () => {
  resetStore();
...
await check('GET by id returns single row', async () => {
  resetStore();
  await POST(postRequest(validBody));
