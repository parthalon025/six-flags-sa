...
await score('B', 'B8', 'the things she must tap are big enough to tap (44px target floor)', async () => {
  ...
  const small = await b.evaluate(() => {
    ...
    /* Tab items and the map capsule's controls use centered ::after
       pseudo-elements with max(100%, 44px) — the selection capsule's Walk and
       Close, plus the hint line that is now the sheet's own handle. */
...
await score('B', 'B11', 'a place she taps can be un-tapped', async () => {
  // Selection used to carry a dismiss control on the glance cards; the capsule's
  // Close button is that answer now — same job, said over the map where the pin is.
