...
const {
  ...
  publicSnapshot,
  reduce,
} = await import('../../apps/party-tracker/lib/core/state.js');
const {
  BYE,
  CLAIM,
  EVERYONE,
  ...
  frame,
  isValidFrame,
} = await import('../../apps/party-tracker/lib/core/protocol.js');
...
const {
  ...
  sheetPlan,
  sheetStops,
} = await import('../../apps/party-tracker/lib/sheet.js');
const { areaOf, centroidOf, clipToBounds, pointInRing, round, simplify } = await import(
  '../../packages/venue-builder/lib/geometry.mjs'
);
const {
  LAYER_RULES,
  POI_RULES,
  ...
  isVenueOutline,
  wayAttributes,
} = await import('../../packages/venue-builder/lib/osm-tags.mjs');
const { WAY_FLAGS, hasWayFlag, wayFlagsOf, wayLayerOf } = await import('../../apps/party-tracker/lib/wayFlags.js');
// The layer builder, for the round trip from raw tags to the bytes that ship.
const { buildLayers } = await import('../../packages/venue-builder/bin/build-venue.mjs');
...
const PARK = JSON.parse(
  fs.readFileSync(new URL('../../apps/party-tracker/public/venues/kings-island.map.json', import.meta.url), 'utf8'),
);
const RIDES = JSON.parse(
  fs.readFileSync(new URL('../../apps/party-tracker/public/venues/kings-island.pois.json', import.meta.url), 'utf8'),
);
const poi = (name) => {
...
await check('every named piece of track belongs to a ride we know', () => {
  const venues = JSON.parse(
    fs.readFileSync(new URL('../../apps/party-tracker/public/venues/manifest.json', import.meta.url)),
  ).venues;
  assert.ok(venues.length, 'no venues to check');
  venues.forEach((v) => {
    const read = (rel) =>
      JSON.parse(fs.readFileSync(new URL(`../../apps/party-tracker/public${rel}`, import.meta.url)));
...
const readVenues = () =>
  JSON.parse(fs.readFileSync(new URL('../../apps/party-tracker/public/venues/manifest.json', import.meta.url))).venues;
const readPois = (rel) => JSON.parse(fs.readFileSync(new URL(`../../apps/party-tracker/public${rel}`, import.meta.url)));

/* ---------------------------------------------------- the venue checklist -- */

const { checklist, failures } = await import('../../packages/venue-builder/lib/venue-checklist.mjs');
const {
...
const { argsFromRecipe, recipeFrom, SHAPING_FLAGS } = await import('../../packages/venue-builder/lib/venue-recipe.mjs');
...
const { renderBrief, requests } = await import('../../packages/venue-builder/lib/venue-requests.mjs');
...
const { compare, crossValidate, fit, project, residuals } = await import('../../packages/venue-builder/lib/georef.mjs');
...
const { applyTrace } = await import('../../packages/venue-builder/lib/venue-trace.mjs');
...
const { atLeast, bandOf, fuse, pointOf, staleness } = await import('../../packages/venue-builder/lib/evidence.mjs');
...
const { addEvidence, attractionFor, claimFromSrc, publishable, SRC_BY, unresolved } =
  await import('../../packages/venue-builder/lib/attractions.mjs');
const { candidates, rideNameOf } = await import('../../packages/venue-builder/lib/candidates.mjs');
...
const { fromTrace, fromTracedFile, inventory, publish } = await import('../../packages/venue-builder/bin/attractions.mjs');
const { OVERRIDE_DIR, VENUE_DIR } = await import('../../packages/venue-builder/lib/venue-io.mjs');
...
const { heightFromTags, poisFromTrack, entrancesFromQueues } = await import('../../packages/venue-builder/bin/build-venue.mjs');
...
const { applyCamping, mergeDataset, readDataset } = await import('../../packages/venue-builder/bin/build-venue.mjs');
...
const { applyImagery } = await import('../../packages/venue-builder/lib/venue-imagery.mjs');
...
await check('readSources resolves recipe-relative catalogue paths from the builder root', () => {
  const { data } = readSources('cedar-point', 'data/venues/cedar-point/sources.json');
  assert.ok(data?.datasets?.imagery?.length, 'recipe-style path must find the catalogue when cwd is the monorepo root');
  return true;
});
...
} = await import('../../packages/venue-builder/lib/venue-judge.mjs');
const { llmConfig } = await import('../../packages/venue-builder/lib/venue-llm.mjs');
...
await check('sourcing plan lists what the catalogue already covers', () => {
  ...
  const map = JSON.parse(fs.readFileSync(new URL(`../../apps/party-tracker/public${venue.map}`, import.meta.url)));
...
const {
  ...
  loadFixtureHtml,
} = await import('../../packages/venue-builder/lib/venue-official-site.mjs');
...
await check('slug-key overrides resolve via addressBook, not display name', async () => {
  const { requests: reqsFn } = await import('../../packages/venue-builder/lib/venue-requests.mjs');
...
await check('every place in every shipped venue gets a unique id', () => {
  const manifest = JSON.parse(
    fs.readFileSync(new URL('../../apps/party-tracker/public/venues/manifest.json', import.meta.url), 'utf8'),
  );
  assert.ok(manifest.venues.length > 0);
  for (const v of manifest.venues) {
    const pois = JSON.parse(
      fs.readFileSync(new URL(`../../apps/party-tracker/public/venues/${v.id}.pois.json`, import.meta.url), 'utf8'),
    );
...
await check('inject-version stamps public/app-version.json from package.json', () => {
  const pkg = JSON.parse(fs.readFileSync(new URL('../../apps/party-tracker/package.json', import.meta.url), 'utf8'));
  const stamped = JSON.parse(
    fs.readFileSync(new URL('../../apps/party-tracker/public/app-version.json', import.meta.url), 'utf8'),
  );
  assert.equal(stamped.version, pkg.version);
  assert.equal(typeof stamped.protocol, 'number');
  assert.ok(stamped.built);
  const sw = fs.readFileSync(new URL('../../apps/party-tracker/public/sw.js', import.meta.url), 'utf8');
...
const {
  normalizeCatalog,
  releaseNotesSince,
} = await import('../../apps/party-tracker/lib/releaseNotes.js');
...
await check('the shipped release-notes file has an entry for the current version', () => {
  ...
  const shipped = JSON.parse(
    fs.readFileSync(new URL('../../apps/party-tracker/data/release-notes.json', import.meta.url), 'utf8'),
  );
...
const { getAdapter, registrySummary, ADAPTER_REGISTRY } = await import('../../packages/venue-builder/lib/adapters/index.mjs');
const { graphFromAttractions, graphFromSidecar, convergenceReport } = await import('../../packages/venue-builder/lib/evidence-graph.mjs');
const { entranceMeta, entranceLine, bestEntrance, atLeastBand } = await import('../../apps/party-tracker/lib/entrance.js');
...
await check('cedar point passes route gate when Siren\'s Curse connector is on the network', () => {
