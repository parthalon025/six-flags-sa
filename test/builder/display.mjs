...
await check('resolveKit paints a kit at a band, and validates what the band said', async () => {
  ...
  // The block is consumed here too: a resolved kit is what the painter holds,
  // and it must not still be able to look up another band's paint.
  for (const band of [null, 'overview', 'mid', 'close']) {
    assert.ok(!('bands' in resolveKit(spec, { assets, band })), `resolved kit leaks bands at ${band}`);
  }
  assert.throws(
...
await check('the seven shipped kits still certify clause 1 — every kind glyphs, at every band', async () => {
  ...
  // Every band, not only the base spec (slice h14): a kit's per-band look is a
  // partial merged over it, so a band could in principle rebind a badge icon
  // to something with a letter on it. Clause 1 is "no band bakes text", which
  // is a claim about all three bands and not about the spec they share.
  const bands = [null, ...BANDS.map((b) => b.id)];
  for (const file of files) {
    for (const band of bands) {
      const spec = JSON.parse(readFileSync(new URL(file, KIT_DIR), 'utf8'));
      const kit = resolveKit(spec, { assets, materials, band });
