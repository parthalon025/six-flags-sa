...
This project is indexed by GitNexus as **six-flags-sa** (4699 symbols, 11678 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
Concurrent Cloud Agent tasks share `/workspace`. Do not `git checkout` there — use an isolated worktree (`git worktree add /tmp/wt-<task> HEAD`) so another task cannot clobber uncommitted files. Apply the `agent-handoff` label when filing.
