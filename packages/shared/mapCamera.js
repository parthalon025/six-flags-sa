 ...
 * allows. Everything else is the kit's job, which is exactly the bet clause 6
 * sends pixel-tycoon first to test.
 *
 ...
 * at mount (`mountMapView({ maxPitch })`), and never per frame — a caller that
 * could set pitch per frame could land a tilt and a restyle in one instant,
 * which is the thing clause 4 exists to prevent.
 *
 ...
 *
 *  The three here are ADR-0021 clause 6's first ship. They are declared as a
 *  contrasting set rather than one by one: an atlas and a painted-paper
 *  brochure are things you read flat, so they take the bottom of clause 2's
 *  range, and pixel-tycoon takes the top plus the turn. That contrast is
 *  itself part of what makes the three read as three worlds — on an axis
 *  `skin-distinct.mjs` cannot see, which is why it is written down here. */
