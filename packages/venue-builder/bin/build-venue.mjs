...
import { OVERRIDE_DIR, gapsDocumentFor, readJson, readOverrides, reindex, resolveBuilderPath, serializeVenue, slugify, VENUE_DIR, writeVenue, venueSidecar } from '../lib/venue-io.mjs';
...
async function buildOne(args, { previous = null } = {}) {
  ...
  if (args.imagery) {
    ...
    for (const file of files) {
      const resolved = resolveBuilderPath(file);
      const collection = JSON.parse(readFileSync(resolved, 'utf8'));
