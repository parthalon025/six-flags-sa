 ...
 * passing pair "may be passing on a single axis".
 *
...
/** Score one pair from both sides. The whole gate, for two Skins. */
async function scorePair(a, b) {
  const spec = specAxesDiffering(
    JSON.parse(readFileSync(kitPath(a), 'utf8')),
    JSON.parse(readFileSync(kitPath(b), 'utf8')),
  );
  const pixel = await pixelAxisDeltas(bakePath(a), bakePath(b));
  // The "never earned" banner is only true if PIXEL_MEASURED still names
  // exactly what pixelAxisDeltas returns. Asserted on every run, not just --null.
  assertPixelMeasuredMatches(pixel);
  return { a, b, spec, pixel, verdict: verdict({ spec, pixel, thresholds: THRESHOLDS }) };
}
...
if (asSet) {
  ...
  let pairs;
  try {
    pairs = skinSetPairs(skins);
  } catch (err) {
    console.error(err.message);
    process.exit(2);
  }
  const scored = [];
  for (const [a, b] of pairs) scored.push(await scorePair(a, b));
  const result = setVerdict(scored);

  if (asJson) {
    console.log(JSON.stringify({
      venue,
      skins,
      pairs: scored.map(({ a, b, spec, pixel, verdict: v }) => ({ a, b, spec, pixel, ...v })),
  ...
  if (result.failing.length) {
    console.log(`\n  Proven not distinct: ${result.failing.map(([a, b]) => `${a} vs ${b}`).join(', ')}`);
  }
  if (result.unproven.length) {
    console.log(`\n  The instrument could not decide: ${result.unproven.map(([a, b]) => `${a} vs ${b}`).join(', ')}`);
  }
  if (result.reason) console.log(`\n  ${result.reason}`);
  console.log(`\n  ${result.outcome} — a set is only as distinct as its closest pair\n`);
  process.exit(exitFor(result.outcome));
}
