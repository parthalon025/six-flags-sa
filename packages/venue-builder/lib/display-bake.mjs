...
/**
 * The geo bounds of a whole cell grid, WSEN — what the raster tier, the
 * pyramid and the pack manifest place the emitted image on.
 *
 * The grid's own far corner is `[cols, rows]`, not `[cols - 1, rows - 1]`:
 * `toGeo` maps a cell's top-left, so the bottom-right cell's far edge is one
 * cell beyond its origin. Off by one here and every tile of the pyramid is
 * georeferenced a cell short.
 */
 ...
 *
 * The model covers the venue's whole planned extent: what `bandBakePlan`
 * describes is what the painter emits, cell for cell. This used to be trimmed
 * to the boundary ring's box plus a margin, so a venue whose boundary left
 * slack inside its bbox planned one picture and baked a smaller one —
 * big-kahunas planned 244x276 and emitted 157x191, and kings-island agreed
 * only because its boundary happens to fill its bbox. The moment tiles are
 * georeferenced that gap is a correctness bug, so the trim is gone rather than
 * taught to the planner: plan and picture are one extent.
 *
...
export function bakeModel(map, pois = [], opts = {}) {
  ...
  const model = {
    ...
    // The picture is the whole planned extent, so its geo bounds are the grid's
    // own corners — the raster tier, the pyramid and the pack manifest all
    // place the image on these.
