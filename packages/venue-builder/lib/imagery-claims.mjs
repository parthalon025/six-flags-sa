 ...
 *
 * Two things gate the deterministic lane, and neither is a flag it sets about
 * itself. Its output digests have to agree across at least two runs
 * (`determinismProof`), and it must not route through an OpenCV primitive whose
 * RNG the caller cannot pin without declaring the mitigations for it
 * (`RNG_TAINTED_PRIMITIVES`). ADR-0020 clause 3 is explicit that determinism is
 * "proven per pass, never assumed"; a boolean is an assumption written down.
 *
 ...
 *
 * Kept as the record CI writes, and deliberately no longer the gate. An id on a
 * list is a claim about a check; the digests are the check, and
 * `determinismProof` reads those instead (ADR-0020 clause 3 — determinism is
 * proven per pass, never assumed). Enrolling a pass here proves nothing on its
 * own, and un-enrolling one hides nothing.
 *
 ...
 * have to change — record a second run, declare the mitigation — rather than
 * leaving a maintainer to infer it from a boolean.
 */
...
export function compareToOsm(extraction, { map, pois = [], toleranceM = DISPUTE_TOLERANCE_M } = {}) {
  ...
  if (extraction.kind === PLACE_KIND) {
    // Beyond four tolerances an OSM Place is a different Place rather than the
    // same one in the wrong spot, so a proximity match out there is imagery
    // adding what OSM lacks (clause 5) and must not be recorded as a dispute.
    const counterpart = counterpartFor(extraction, pois, toleranceM * 4);
