...
export const TEST_ESTATE_EXCLUDED = {
  ...
  'test/app/ux.mjs':
    'human-run exploration (`npm run test:ux`): boots a phone viewport, logs page errors and takes two sheet-peek screenshots. It asserts nothing, so running it in CI would only prove a browser started. It is in modules.json fullSuitePaths, so editing it still forces the full UI matrix.',
};
